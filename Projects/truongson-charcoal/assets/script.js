...
(function () {
  ...
  window.addEventListener('load', () => setTimeout(() => pre && pre.classList.add('done'), 1100));
...
/* ===== NAV / PROGRESS / REVEAL / COUNTERS (scroll-driven) ===== */
...
function frameUpdate() {
  ...
  for (let i = counterEls.length - 1; i >= 0; i--) {
    if (counterEls[i].getBoundingClientRect().top < vh * 0.85) {
      animateCounter(counterEls[i]); counterEls.splice(i, 1);
    }
  }
  let current = null;
  ...
  Object.entries(linkMap).forEach(([id, a]) => {
    if (!a.classList.contains('nav__cta')) a.style.color = (id === current) ? 'var(--cream)' : '';
  });
}
...
form.addEventListener('submit', (e) => {
  ...
  if (!name || !email || !product) {
    [['name', name], ['email', email], ['product', product]].forEach(([id, v]) => {
  ...
  setTimeout(() => {
    ...
    form.querySelector('button span').textContent = 'Gửi Yêu Cầu Báo Giá';
  }, 4800);
});
